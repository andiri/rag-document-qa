...
ETF 관점(개인 투자자 관심사): 자료 곳곳에 흩어진 ETF·펀드 관련 신호를 놓치지 말고 모아 준다.
- 국내외 ETF 자금 유출입(순유입·순유출)과 그 배경(금리·환율·섹터 로테이션),
- 신규 상장·테마(반도체·2차전지·AI·방산·리츠 등) ETF 및 관련 지수 흐름,
- 연금·ISA 등 장기투자 계좌에서 주목받는 ETF 흐름.
관련 근거가 자료에 있을 때만 다루고, 없으면 억지로 만들지 않는다.`;

const INSTRUCTION = `위 자료를 바탕으로 "오늘의 한경 브리핑"을 분야별로 작성하세요. 충분히 읽을거리가 되도록 조금 길고 자세하게 씁니다.

- headline: 오늘 한경의 가장 큰 이슈 한 줄(45자 이내).
- tldr: 분야를 통틀어 오늘 꼭 알아야 할 핵심 6~9개 불릿(각 70자 이내). ETF·펀드 자금 흐름 신호가 있으면 최소 1개는 포함한다.
- sections: 자료에 등장한 분야만 포함(증시/ETF/부동산/산업/정책/종합 등). 각 분야는
    · summary: 그 분야 오늘의 흐름을 2~3문장으로 요약(맥락·배경 포함),
    · items: 그 분야의 주요 이슈 3~5개. 각 항목은 title(이슈 제목)과 insight(왜 중요한지/배경/파급효과를 1~2문장으로).
- ETF 섹션: 자료에 ETF·펀드·상장지수펀드·자금 유출입 관련 내용이 있으면 category "ETF" 섹션을 증시 바로 다음에 넣는다.
  자금 흐름의 방향과 배경, 주목받는 테마·신규 상장 ETF, 국내외 ETF 시장 온도차를 items로 정리한다. 근거가 없으면 이 섹션은 생략한다.`;
